...
DATE="$(date +%F)"
RUN_ID="${RUN_ID:-smoke-solve-$(date +%H%M%S)}"
SMOKE_SAVE_DIR="${SMOKE_SAVE_DIR:-${RUNS_DIR}/_smoke}"
SMOKE_RUN_DIR="${SMOKE_SAVE_DIR}/${RUNNER_PROFILE}/date-${DATE}/runid-${RUN_ID}__solve__${PRESET}"

SMOKE_KEEP="${SMOKE_KEEP:-0}"
SMOKE_KEEP_ON_FAIL="${SMOKE_KEEP_ON_FAIL:-1}"

# Pre-check CUDA
"${PYTHON_BIN}" -c "import os, sys, torch; req=str(os.environ.get('CVDC_REQUIRE_CUDA','0')).strip()=='1'; print('[smoke:solve] CVDC_REQUIRE_CUDA=' + str(os.environ.get('CVDC_REQUIRE_CUDA'))); print('[smoke:solve] CUDA_VISIBLE_DEVICES=' + str(os.environ.get('CUDA_VISIBLE_DEVICES'))); print('[smoke:solve] torch.cuda.is_available=' + str(torch.cuda.is_available())); print('[smoke:solve] torch.cuda.device_count=' + str(torch.cuda.device_count())); sys.exit(0 if (not req or torch.cuda.is_available()) else 2)"
...
echo "[smoke:solve] run_dir=${SMOKE_RUN_DIR}"

if [[ ! -f "${SMOKE_RUN_DIR}/train.json" || ! -f "${SMOKE_RUN_DIR}/eval.json" ]]; then
  echo "[smoke:solve] missing train.json or eval.json; keeping artifacts for debugging" >&2
  SMOKE_KEEP=1
fi
...
PY
  if [[ $? -ne 0 ]]; then
    echo "[smoke:solve] invalid eval.json; keeping artifacts for debugging" >&2
    SMOKE_KEEP=1
  fi
fi

if [[ ${RC} -ne 0 ]]; then
  echo "[smoke:solve] FAILED (rc=${RC})" >&2
  if [[ "${SMOKE_KEEP_ON_FAIL}" != "1" ]]; then
    echo "[smoke:solve] cleanup_on_fail enabled; removing ${SMOKE_RUN_DIR}" >&2
    rm -rf "${SMOKE_RUN_DIR}" || true
  else
    echo "[smoke:solve] keeping artifacts for debugging (set SMOKE_KEEP_ON_FAIL=0 to remove)" >&2
  fi
  exit ${RC}
fi

if [[ "${SMOKE_KEEP}" == "1" ]]; then
  echo "[smoke:solve] success; keeping artifacts (SMOKE_KEEP=1)"
  exit 0
fi
...
case "${SMOKE_RUN_DIR}" in
  "${RUNS_DIR}"/_smoke/*) ;;
  *)
    echo "[smoke:solve] ERROR: cleanup guard triggered - SMOKE_RUN_DIR is not under RUNS_DIR/_smoke/" >&2
    echo "[smoke:solve] SMOKE_RUN_DIR=${SMOKE_RUN_DIR}, RUNS_DIR=${RUNS_DIR}" >&2
    exit 2
    ;;
esac

echo "[smoke:solve] success; cleaning up ${SMOKE_RUN_DIR}"
rm -rf "${SMOKE_RUN_DIR}"
